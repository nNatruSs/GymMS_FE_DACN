# Explanation: `src/guard/admin.guard.ts`

## File Overview
- **Relative path:** `src/guard/admin.guard.ts`
- **Total lines:** `43`
- **Non-empty lines:** `16`
- **Import statements:** `3`
...
| 28 | `export const adminGuard: CanMatchFn = (route, segments) => {` | Exports symbols so other files can import this logic. |
...
| 37 | `  if (storage.isAdminLoggedIn()) {` | Conditional branch: executes block only when condition is true. |
